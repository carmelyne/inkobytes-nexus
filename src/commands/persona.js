/**
 * nexus persona - apply a local persona overlay to agent instruction files
 */
...
const DEFAULT_OVERLAY = `# Local Persona Overlay

Add your local agent persona notes here. This file is local workspace flavor:
relationship tone, preferred response rhythm, naming defaults, and other
private context that should sit outside the public Nexus protocol block.
`;
...
function parseArgs(args) {
  ...
  if (!args[fileIndex + 1]) {
    throw new Error('Usage: nexus persona [--file <path>]');
  }
  return { overlayPath: args[fileIndex + 1] };
}

function localPersonaBlock(overlayPath, content) {
  return `${START_MARKER_PREFIX} ${overlayPath} -->
${content.trim()}
${END_MARKER}`;
}

function upsertPersonaBlock(existing, overlayPath, overlayContent) {
  const start = existing.indexOf(START_MARKER_PREFIX);
  const end = existing.indexOf(END_MARKER);
  const block = localPersonaBlock(overlayPath, overlayContent);
...
export default function persona(args) {
  const root = cwd();
  ...
  for (const entrypoint of AGENT_ENTRYPOINTS) {
    ...
    const next = upsertPersonaBlock(existing, overlayPath, overlayContent);
  ...
  if (updated.length) {
    console.log('Applied local persona overlay:');
    for (const entrypoint of updated) console.log(`  - ${entrypoint}`);
  } else {
    console.log('Local persona overlay already up to date.');
  }

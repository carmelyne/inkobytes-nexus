...
**Human-controlled:** only verified sessions (`CLAUDECODE=1` or `NEXUS_AGENT` set) may run `nexus chmod`. This is an advisory contract honored at session start, not mechanical enforcement — the gate is environment variables any process can set.
...
- **Level 0 — supervised.** A human watches the session and approves each significant step. `nexus release --no-verify` is allowed here and only here.
- **Level 1 — checkpointed.** Agents work the queue between human checkpoints. Prerequisite: `release.verifyCommand` must be configured; every release proves itself, and `--no-verify` is refused.
- **Level 2 — bounded unattended.** Agents run without a human present, inside explicit volume bounds. Prerequisites: everything from Level 1, plus `.nexus/agent-budgets.json` for active agents. Until a `nexus recover` command exists, rollback after unattended mistakes is manual git work — doctor says so honestly.

**Human checkpoint ritual.** At each checkpoint the human:

1. Reads `_NEXUS_STANDUP.md` lines since the last checkpoint.
2. Reviews release receipts in `_NEXUS_REPORT.md`.
3. Runs `nexus doctor` and `nexus status`.
4. Decides: continue, adjust the queue, or `nexus halt`.

`nexus start` reports the current level so agents know the operating mode before claiming work. `nexus doctor` warns when a level's prerequisites are missing.

**Honesty rule:** changing the level is human-only by convention. Nexus does not mechanically prevent an agent from editing `.nexus/config.json` — doctor reports prerequisite gaps, and agents must treat the level as human-owned. Never claim enforcement the code does not deliver.

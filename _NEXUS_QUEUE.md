...
- [ ] TASK/Codex: Add delegated queue lanes with batch reconciliation
  ...
  - Notes: Copy, do not move. The master queue keeps task id, status, delegation pointer, lane, and reconciliation state. Agent lanes carry active notes, blockers, local substeps, and done receipts. `_NEXUS_QUEUE.md` should not be mutated by agents during active work. Reconciliation is explicit, batchable, and recoverable; decide whether it is a human checkpoint ritual, agent-runnable at autonomy 1+, or both. Done is not a master write; done is a receipt. `--take` must copy the full task block, including Goal/Outcome/Constraints/Stop If/Evidence primitives, into the lane and reuse `src/lib/taskContract.js` so under-specified tasks cannot be delegated at autonomy 1+. Flag lane claim semantics as a constitution-level decision: owner-exempt lanes with `nexus q` handling commits internally may be needed to avoid recreating claim/release overhead. Update README, bundled Nexus Skill, constitution, and `src/lib/protocolText.js` so doctor/init can regenerate agent files with the new lane workflow. Phase 1 landed 2026-06-11: `nexus next --take`, `nexus q`, `nexus q done`, lane helpers, shell completion, README, bundled Skill, and generated protocol text; `npm test` passed 213/213. Phase 2 remains: `nexus queue reconcile`, `nexus doctor` warnings for unreconciled receipts/stale lanes/master-lane disagreement, constitution update, and reconciliation tests. This absorbs the core assignment-state goals of `task-state-commands` while avoiding a new completion bottleneck.
